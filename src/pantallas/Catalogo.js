...
export default function Catalogo({ navigation }) {
  ...
  // Incrementa o decrementar la cantidad del producto
  const modificarCantidad = (id, cambio) => {
    ...
    // Valida el tope maximo de las 20 unidades x item
    if (nueva > 20) {
  ...
  // Valida seleccion antes de pasar a la orden
  const irAOrden = () => {
    if (totalUnidades === 0) {
...
const styles = StyleSheet.create({
  contenedor: { flex: 1, backgroundColor: colores.verde, padding: espaciado.md },
  contenedorTabs: {
    flexDirection: 'row',
    marginBottom: espaciado.md,
    backgroundColor: colores.verdeClaro,
  },
  tab: { flex: 1, paddingVertical: espaciado.sm, alignItems: 'center' },
  tabActiva: { backgroundColor: colores.acento },
  tabTexto: { color: colores.cremaOpaco, fontWeight: '600' },
  tabTextoActivo: { color: colores.crema, fontWeight: '700' },
  errorText: {
    color: colores.error,
    backgroundColor: 'rgba(226, 148, 107, 0.15)',
  ...
  info: { flex: 1, marginLeft: espaciado.md },
  nombre: { color: colores.crema, fontSize: 15, fontWeight: '600' },
  precio: { color: colores.cremaOpaco, fontSize: 14, marginTop: 2 },
  selector: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  textoControl: { color: colores.crema, fontSize: 16, fontWeight: '700' },
  cantidadTexto: {
    color: colores.crema,
